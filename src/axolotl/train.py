...
def train(
    *, cfg: DictDefault, cli_args: TrainerCliArgs, dataset_meta: TrainDatasetMeta
    ...
    send_http_payload({ "status": "LOADING TOKENIZER" })
    LOG.debug(
    ...
    if cfg.resume_from_checkpoint is None and cfg.auto_resume_from_checkpoints:
        ...
        if len(possible_checkpoints) > 0:
            send_http_payload({ "status": "LOADING MODEL FROM CHECKPOINT" })
            sorted_paths = sorted(
                possible_checkpoints,
    ...
    if peft_config:
        send_http_payload({ "status": "PRE-SAVING ADAPTER" })
    ...
    send_http_payload({ "status": "STARTING TRAINER" })
    if cfg.group_by_length:
        LOG.info("hang tight... sorting dataset for group_by_length")

    send_http_payload({ "status": "TRAINING" })
